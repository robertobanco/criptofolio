        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card title="Valor Total do Portfólio">
                <div className="flex justify-between items-baseline">
                    <span className="text-indigo-400">R$ {totalPortfolioValue.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
             ...
             <Card title="Custo Base Total (Investido)">
                <span className="text-indigo-400">R$ {totalCostBasis.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </Card>
            <Card title="Lucro/Prejuízo Não Realizado">
                <div className={unrealizedProfit >= 0 ? 'text-green-400' : 'text-red-400'}>
                    <span>R$ {unrealizedProfit.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    <span className="text-base font-medium ml-2">({unrealizedProfitPercentage.toFixed(2)}%)</span>
                </div>
            ...
            <Card title="Variação em 24h">
                <span className={total24hChangeValue >= 0 ? 'text-green-400' : 'text-red-400'}>
                    {total24hChangeValue >= 0 ? '+' : ''}R$ {total24hChangeValue.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                    <span className="text-sm ml-2">({total24hChangePercent.toFixed(2)}%)</span>
                </span>
            </Card>
                ...
                <table className="w-full text-sm">
                    ...
                    {sortedTableData.map(asset => (
                        ...
                        >
                            ...
                            <td className="p-2 text-right">R$ {asset.currentValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className={`p-2 text-right font-semibold ${asset.profitLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}>R$ {asset.profitLoss.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            <td className={`p-2 text-right font-semibold ${asset.variation >= 0 ? 'text-green-400' : 'text-red-400'}`}>{asset.variation.toFixed(2)}%</td>
                            <td className={`p-2 text-right font-semibold ${asset.change24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>{asset.change24h.toFixed(2)}%</td>
                        </tr>
                    ))}
                    </tbody>
                </table>
